    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Row(equal_height=True):
                ...
                # 定义 LoRA 模型目录路径
                LORA_BASE_DIR = r"D:\AIJOB\ACE-Step-ST\checkpoints"

                def get_local_lora_models(base_dir):
                    """扫描 base_dir 下的所有子目录，检查是否存在 LoRA 权重文件"""
                    lora_dirs = []
                    if not os.path.isdir(base_dir):
                        print(f"[WARNING] LoRA base dir does not exist: {base_dir}")
                        return []
                    ...
                    for item in os.listdir(base_dir):
                        ...
                        if os.path.isdir(path):
                            # 检查是否存在 LoRA 权重文件
                            safetensor_path = os.path.join(path, "pytorch_lora_weights.safetensors")
                            bin_path = os.path.join(path, "pytorch_lora_weights.bin")
                            if os.path.exists(safetensor_path) or os.path.exists(bin_path):
                                lora_dirs.append(path)
                    return lora_dirs

                # 动态获取本地 LoRA 模型列表
                local_lora_choices = get_local_lora_models(LORA_BASE_DIR)

                # 合并预设模型 + 本地模型
                all_lora_choices = ["none"] + local_lora_choices

                lora_name_or_path = gr.Dropdown(
                    label="Lora Name or Path",
                    choices=all_lora_choices,
                    value="none",
